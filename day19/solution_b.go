...
func algo(bluePrint Blueprint, r Resources, timeLeft int, step int) int {
	if timeLeft == 1 {
	...
	for _, robot := range run {
		...
		if step >= 15 && robot == "ore" { // && r.robots[robot] >= bluePrint.maxCosts[robot] / 2) {
			continue
		}
		if step >= 18 && robot == "clay" { // && r.robots[robot] >= bluePrint.maxCosts[robot] / 2) {
			continue
		}
		...
		if gotall {
			if robot == "geode" {
				...
				if step >= 17 && robot == "obsidian" && (r.product["ore"] + r.robots["ore"]) >=
				   (bluePrint.robot["obsidian"]["ore"] + bluePrint.robot["geode"]["ore"])  {
						options = []string{robot}
						break
				} else {
	...
	for _, option := range options {
		...
		localMax := algo(bluePrint, newResources, timeLeft-1, step+1)
		if localMax > max {
			max = localMax
		}
	}
